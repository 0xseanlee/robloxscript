...
local Connections = {
    NoClip = nil,
    AntiTeleportHeartbeat = nil,
    AntiTeleportSignal = nil,
    AntiRagdoll = nil,
    FlyRender = nil,
    InputBegan = nil,
    InputEnded = nil,
...
local function modifyPrompt(prompt)
    if Features.InstantPrompt and prompt:IsA("ProximityPrompt") then
...
Connections.PromptShown = ProximityPromptService.PromptShown:Connect(function(prompt)
    modifyPrompt(prompt)
end)
...
local function updateJointsCache(char)
    cachedJoints = {}
    for _, joint in pairs(char:GetDescendants()) do
        if joint:IsA("Motor6D") then
            cachedJoints[joint.Name] = {
                Part0 = joint.Part0,
                Part1 = joint.Part1,
                C0 = joint.C0,
                C1 = joint.C1
...
local function restoreJoints()
    for name, data in pairs(cachedJoints) do
        if data.Part0 and data.Part0:FindFirstChild(name) == nil then
...
local function setupAntiTeleport()
    if Connections.AntiTeleportHeartbeat then Connections.AntiTeleportHeartbeat:Disconnect() end
    if Connections.AntiTeleportSignal then Connections.AntiTeleportSignal:Disconnect() end
    if not Features.AntiTeleport then return end
    ...
    Connections.AntiTeleportHeartbeat = RunService.Heartbeat:Connect(function()
        if not isReverting and hrp then
            lastCF = hrp.CFrame
        end
    end)

    Connections.AntiTeleportSignal = hrp:GetPropertyChangedSignal("CFrame"):Connect(function()
        if Features.AntiTeleport and not isReverting and lastCF then
            isReverting = true
            hrp.CFrame = lastCF
...
local function onCharacterAdded(char)
    ...
    updateJointsCache(char)

    if Features.NoClip then setupNoClip() end
    if Features.AntiTeleport then setupAntiTeleport() end
    if Features.AntiRagdoll then setupAntiRagdoll() end
    if Features.Fly then startFlying(flySpeedMultiplier) end

    hum.Died:Connect(function()
        stopFlying()
        for k, conn in pairs(Connections) do
            if conn and k ~= "PromptShown" and k ~= "DescendantAdded" then 
                conn:Disconnect() 
            end
        end
    end)
...
function toggleAntiTeleport(state)
    Features.AntiTeleport = (state ~= nil) and state or not Features.AntiTeleport
    setupAntiTeleport()
end

function toggleAntiRagdoll(state)
    Features.AntiRagdoll = (state ~= nil) and state or not Features.AntiRagdoll
    setupAntiRagdoll()
end

function toggleInstantPrompt(state)
    Features.InstantPrompt = (state ~= nil) and state or not Features.InstantPrompt
